...
interface KBPageNavigationProps {
  tenantId: string;
...
export function KBPageNavigation({ 
  tenantId, 
  documentCount = 0, 
  referenceCount = 0,
  className 
}: KBPageNavigationProps) {
  ...
  const renderNavItem = (item: NavItem, level: number = 0) => {
    ...
    return (
      <div key={item.id}>
        <div
          className={cn(
            "flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition-colors cursor-pointer group",
            active && "bg-blue-100 dark:bg-blue-950 text-blue-700 dark:text-blue-300 shadow-sm",
            !active && "hover:bg-gray-50 dark:hover:bg-gray-800",
            level > 0 && "ml-6 py-2"
          )}
        >
